...
enum { SchemeNorm, SchemeSel, SchemeRed, SchemeYellow, SchemeOrange, SchemeGreen, SchemeBlue }; /* color schemes */
...
void
runAutostart(void) {
	system("cd /home/steve/.dwm; ./autostart.sh &");
}
...
setup(void)
{
	...
	lrpad = drw->fonts->h + barpadw;
	bh = drw->fonts->h + 2 + barpadh;
	updategeom();
	/* init atoms */
